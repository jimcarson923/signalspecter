...
export async function registerRoutes(httpServer: ReturnType<typeof createServer>, app: Express) {
  ...
  app.get('/api/chart/:symbol', async (req, res) => {
    ...
    try {
      ...
      const polyData = await polyRes.json() as { results?: Array<{ t:number; o:number; h:number; l:number; c:number; v:number }> };
      ...
      const candles: Candle[] = polyData.results.map(c => ({ t:c.t, o:c.o, h:c.h, l:c.l, c:c.c, v:c.v }));

      const closes: number[] = candles.map(c => c.c);
      const highs:  number[] = candles.map(c => c.h);
      const lows:   number[] = candles.map(c => c.l);
      ...
      const rr     = +((target - entry) / Math.max(entry - stop, 0.01)).toFixed(1);

      const slice20 = closes.slice(-Math.min(20, closes.length));
      const sma20   = +(slice20.reduce((a, b) => a + b, 0) / slice20.length).toFixed(2);
      ...
      if (openaiKey) {
        try {
          const prompt = 'Ticker: ' + symbol + '. Timeframe: ' + timeframe + '. Price: $' + currentPrice + '. Trend: ' + trend + ' (SMA20 $' + sma20 + '). Support: $' + support1 + '. Resistance: $' + resistance1 + '. Buy zone: $' + buyZoneLow + '-$' + buyZoneHigh + '. Target: $' + target + '. Stop: $' + stop + '. R/R: ' + rr + ':1.';
          const aiRes  = await fetch('https://api.openai.com/v1/chat/completions', {
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + openaiKey, 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ...
              max_tokens: 180,
              messages: [
                { role: 'system', content: 'You are Specter, elite AI trading analyst. Give a sharp 3-sentence chart plan: trend, buy zone, target, stop. Sound like a confident pro, not a robot.' },
      ...
      res.json({ symbol, timeframe, currentPrice, trend, sma20, candles, levels: { support1, support2, resistance1, resistance2 }, buyZone: { low: buyZoneLow, high: buyZoneHigh }, sellZone: { low: sellZoneLow, high: sellZoneHigh }, plan: { entry, target, stop, rr }, narrative });
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Unknown error';
      res.status(500).json({ error: 'Chart data fetch failed', detail: msg });
    }
  });
